...
def normalize(inputs, epsilon=1e-8, scope="normalize", reuse=None):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        beta = tf.compat.v1.get_variable(name='beta', shape=params_shape, dtype=tf.float32, initializer=tf.zeros_initializer())
        gamma = tf.compat.v1.get_variable(name='scale', shape=params_shape, dtype=tf.float32, initializer=tf.ones_initializer())
...
def embedding(inputs, vocab_size=None, embedding_size=None, zero_pad=False, scale=False, scope="embedding", reuse=None, initializer=None):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        if initializer is not None:
            ...
            lookup_table = tf.compat.v1.get_variable('lookup_table', dtype=tf.float32, shape=[vocab_size, embedding_size], initializer=tf.contrib.layers.xavier_initializer())
        if zero_pad:
            lookup_table = tf.concat((tf.zeros(shape=[1, embedding_size]), lookup_table[1:, :]), 0)
        outputs = tf.compat.v1.nn.embedding_lookup(lookup_table, inputs)
        if scale:
...
def positional_encoding(inputs, num_units, max_len, zero_pad=True, scale=True, scope="positional_encoding", reuse=None):
    ...
    T = max_len
    with tf.compat.v1.variable_scope(scope, reuse=reuse, dtype=tf.float32):
        ...
        outputs = tf.compat.v1.nn.embedding_lookup(lookup_table, position_ind)
        if scale:
...
def feedforward(inputs, num_units=[2048, 512], scope="feed_forward", reuse=None):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        params = {"inputs": inputs, "filters": num_units[0], "kernel_size": 1, "activation": tf.compat.v1.nn.relu, "use_bias": True}
